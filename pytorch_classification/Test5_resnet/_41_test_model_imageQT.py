...
class Picture(QWidget):
    ...
    def readmodel(self):
        try:
            ...
            self.lbl4ModelName.setText("加载模型异常：" + self.model_full_name)
...
class mymodel():
    ...
    def predict(self, imgpath):
        ...
        print(self.class_indict[str(predict_cla)], predict[predict_cla].numpy())
        percent = str(predict[predict_cla].numpy())
        res_name = self.class_indict[str(predict_cla)]
        res_joson = "{0},{1},{2}".format(predict_cla, percent, res_name)
        # res_joson = "{" + '"index":{0},"percent":{1},"type":"{2}"'.format(predict_cla, percent,res_name) + "}"
        return res_joson
